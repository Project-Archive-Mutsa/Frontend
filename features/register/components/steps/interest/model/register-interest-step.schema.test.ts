...
describe("회원가입 관심 분야 검증", () => {
  it("서버에서 받은 양의 정수 파트와 태그 ID를 허용한다", () => {
  ...
  it("관심 태그를 선택하지 않으면 오류를 반환한다", () => {
    ...
    if (!result.success) {
      expect(result.error.flatten().fieldErrors.selectedInterests?.[0]).toBe(
        "관심 태그를 하나 이상 선택해 주세요.",
      );
    }
  });

  it("유효하지 않은 파트 ID를 선택하면 오류를 반환한다", () => {
    const result = registerInterestStepSchema.safeParse({
      selectedInterests: [{ partId: 0, tagId: 66 }],
    });
  ...
  it("유효하지 않은 태그 ID를 선택하면 오류를 반환한다", () => {
    const result = registerInterestStepSchema.safeParse({
      selectedInterests: [{ partId: 5, tagId: 0 }],
    });
