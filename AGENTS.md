...
- 백엔드 이슈에는 프론트 기능과 사용자 흐름, 현재 백엔드 상태, 필요한 엔드포인트와
  ...
  이슈로 뭉치지 않습니다.
- Codex는 `Project-Archive-Mutsa/Backend` 저장소를 읽기·분석하고 GitHub 이슈를
  작성하거나 보완하는 용도로만 사용합니다. 백엔드 파일 수정, 브랜치 전환, 로컬
  체크아웃을 바꾸는 pull·merge·reset, commit, push와 PR 생성은 절대 수행하지
  않습니다. 최신 백엔드 상태는 GitHub API 또는 `gh`의 읽기 전용 조회로 확인합니다.
...
- 백엔드 계약과 기존 이슈 범위는 먼저 `docs/backend-issues.md`에서 확인합니다.
  이 문서는 빠른 조회용 스냅샷이므로 기능 범위를 파악하기 위해 매번 GitHub 이슈를
  다시 조회하지 않습니다.
- 현재 이슈 상태·새 댓글·변경된 본문을 확인하거나 이슈를 작성·보완할 때만 GitHub를
  조회합니다. 조회 결과가 문서와 다르면 관련 요약과 마지막 동기화 날짜를 함께
  갱신하며, 로컬 문서의 상태를 실시간 상태로 단정하지 않습니다.
