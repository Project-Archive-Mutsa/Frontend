...
- 등록자가 공개한 재사용 자산이 있는 프로젝트를 별도로 탐색
- 공개 자산의 종류와 자산별 라이선스 확인
- 상세 진입 전에 프로젝트 등록 정보의 `정보 충실도` 확인
- 기존 시도와 한계를 읽고 새로운 프로젝트로 계승

`좀비 프로젝트`는 확정된 제품 용어다. 개발 상태가 `일시 중단`이라는 이유만으로 좀비 프로젝트가
되는 것은 아니며, 재사용 가능한 자산과 활용 조건을 공개한 별도 등록 목적을 뜻한다.
...
- 상세 리포트 열람, 판매 협의, 저장, 공유와 소유자 연락 같은 후속 행동
- 프로젝트 자산의 권리 또는 인증 정보 표시

상세 리포트 열람권과 프로젝트 자산·권리 판매는 별개의 거래로 표시한다. 가격, 인증 배지와
거래 버튼은 기능 가능성을 표현한 더미 요소이며 실제 구현 전 권리 확인과 거래 정책이 필요하다.
...
문제·해결·결과 → 회고·보유 자산 → 등록 목적 → 검토·등록의 여섯 단계이며, 파일별 임의 공개 토글과
계좌 입력은 사용하지 않는다. 자산은 분야에 맞는 파일과 Figma·GitHub 등 외부 링크를 하나의 논리적
자산으로 묶고, 마지막 단계에서 정보 충실도 점검과 공개 범위를 확인한다.
